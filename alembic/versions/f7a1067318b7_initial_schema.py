...
def upgrade() -> None:
    ...
    # Create contents table (before items, since items references it)
    op.create_table(
        "contents",
    ...
    op.create_table(
        ...
        sa.Column("content_id", sa.String(64), sa.ForeignKey("contents.content_id"), nullable=True),
    ...
    op.create_index("ix_items_content_id", "items", ["content_id"])
...
def downgrade() -> None:
    ...
    # Drop items first (references contents)
    ...
    op.drop_index("ix_items_content_id", table_name="items")
