...
┌─────────────────────────────────────────────────────────┐
│ 2. 配置版本检查                                          │
│    - 解析 sources.yaml 的 version 字段（如有）           │
│    - 与新版本默认配置版本比较                             │
│    - 版本不匹配 → 拒绝升级，提示手动处理                  │
└─────────────────────────────────────────────────────────┘
                         ↓
...
check_config_version() {
    local user_version
    user_version=$(grep "^version:" sources.yaml 2>/dev/null | awk '{print $2}')

    local new_version
    new_version=$(docker run --rm cyberpulse-api:latest cat /app/config/sources.yaml.default | grep "^version:" | awk '{print $2}')

    if [[ "$user_version" != "$new_version" ]]; then
        echo "配置版本不匹配: 当前 v$user_version, 新版本 v$new_version"
        echo "请阅读升级文档，手动调整配置后重试"
        return 1
    fi
    return 0
}
...
| 数据 | 存储位置 | 升级时处理 |
|------|----------|------------|
| PostgreSQL 数据 | Docker volume | 保留，迁移脚本更新 |
| 数据库快照 | `~/.snapshots/` | 升级前自动创建 |
| Admin Key | 数据库（bcrypt 哈希） | 保留 |
| .env | `deploy/.env` | 快照备份，升级后保留 |
| sources.yaml | 项目根目录 | 快照备份，格式兼容时保留 |
