...
- **场景**：开发者在主仓库和 worktree 都在同一分支（如 main）部署
- **结果**：项目名相同（基于分支哈希），容器冲突
- **限制说明**：开发者通常只需部署一个测试环境，此场景罕见
- **规避方法**：如需同时运行，手动设置 `CYBER_PULSE_ENV` 环境变量或 `.cyber-pulse-env` 文件
...
| 主仓库与 worktree 同分支冲突 | 低 | 低 | 文档说明限制，提供规避方法 |
